...
router.post("/", protect, upload.array("images", 10), async (req, res) => {
  try {
    ...
    const property = new Property({
      ...
      securityDetails: securityDetails ? (typeof securityDetails === 'string' ? JSON.parse(securityDetails) : securityDetails) : [],
    });
...
router.get("/nearby", async (req, res) => {
  try {
    ...
    // Approximately 10km radius (1 degree ~= 111km, so 0.09 ~= 10km)
    const radius = 0.09;
